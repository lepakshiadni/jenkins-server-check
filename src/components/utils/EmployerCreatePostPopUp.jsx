...
import '../styles/EmployerCreatePostPopup.css'
...
import { employerCreatePost } from '../../redux/action/employercreatepost.action'




function EmployerCreatePostPopUp(props) {
    ...
    const employercreate = useSelector(({ employerCreatePost }) => {
        return employerCreatePost;
    ...
    const handlePostButtonClick = async () => {
        ...
        dispatch(employerCreatePost(formData));
        await props.setTrigger(false);
    };
    useEffect(() => {
        if (employercreate?.success) {
            toast.success('Post Created SucessFully')
        }
        else if (employer?.success === false) {
            toast.error('Something Went Wrong')
        }
    }, [employercreate?.message]);

    console.log("employercreate", employercreate)



    return props.trigger ? (
        <div className="employercreatePopUp">
            <div className="employercreatePopUpContainer">
                <div
                    style={{
                        display: "flex",
                    ...
                    <div className="name">
                        {employer?.basicInfo?.profileImg ? (
                            ...
                            <div className="w-[50px] h-[50px] bg-[#8888] rounded-full  flex items-center justify-center">
                                <span className="text-lg ">{employer?.fullName[0]}</span>
                            </div>
                        )}
...
export default EmployerCreatePostPopUp;
